import { createClient } from '@supabase/supabase-js'
import { NextResponse } from 'next/server'

const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
)

const ADMIN_EMAIL = 'samoitoza@gmail.com'

// GET - fetch the tour list for a project
export async function GET(req: Request) {
  const { searchParams } = new URL(req.url)
  const projectId = searchParams.get('projectId') || 'sf-office-search'

  const { data, error } = await supabase
    .from('tour_list_items')
    .select('*')
    .eq('project_id', projectId)
    .order('sort_order', { ascending: true })
    .order('created_at', { ascending: true })

  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json(data)
}

// POST - add a building to the tour list (or update schedule)
export async function POST(req: Request) {
  const body = await req.json()
  const {
    projectId = 'sf-office-search',
    buildingType,
    buildingId,
    buildingName,
    buildingAddress = '',
    buildingColor = '#9ca3af',
    buildingMeta = '',
    buildingTags = [],
    lat,
    lng,
    pdfPage,
    tourDate,
    tourTime,
    sortOrder = 0,
    addedBy,
  } = body

  // If only updating schedule (tourDate/tourTime), do an update
  if (tourDate !== undefined || tourTime !== undefined) {
    const updates: Record<string, string> = {}
    if (tourDate !== undefined) updates.tour_date = tourDate
    if (tourTime !== undefined) updates.tour_time = tourTime

    const { data, error } = await supabase
      .from('tour_list_items')
      .update(updates)
      .eq('project_id', projectId)
      .eq('building_type', buildingType)
      .eq('building_id', buildingId)
      .select()
      .single()

    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json(data)
  }

  // Otherwise upsert the full item
  if (!buildingType || buildingId === undefined || !buildingName) {
    return NextResponse.json({ error: 'buildingType, buildingId, and buildingName are required' }, { status: 400 })
  }

  const { data, error } = await supabase
    .from('tour_list_items')
    .upsert({
      project_id: projectId,
      building_type: buildingType,
      building_id: buildingId,
      building_name: buildingName,
      building_address: buildingAddress,
      building_color: buildingColor,
      building_meta: buildingMeta,
      building_tags: buildingTags,
      lat,
      lng,
      pdf_page: pdfPage,
      tour_date: tourDate || '',
      tour_time: tourTime || '',
      sort_order: sortOrder,
      added_by: addedBy,
    }, { onConflict: 'project_id,building_type,building_id' })
    .select()
    .single()

  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json(data)
}

// DELETE - remove a building from the tour list
export async function DELETE(req: Request) {
  const { searchParams } = new URL(req.url)
  const id = searchParams.get('id')
  const projectId = searchParams.get('projectId') || 'sf-office-search'
  const buildingType = searchParams.get('buildingType')
  const buildingId = searchParams.get('buildingId')

  if (id) {
    const { error } = await supabase.from('tour_list_items').delete().eq('id', id)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ success: true })
  }

  if (buildingType && buildingId) {
    const { error } = await supabase
      .from('tour_list_items')
      .delete()
      .eq('project_id', projectId)
      .eq('building_type', buildingType)
      .eq('building_id', Number(buildingId))
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ success: true })
  }

  return NextResponse.json({ error: 'id or buildingType+buildingId required' }, { status: 400 })
}
